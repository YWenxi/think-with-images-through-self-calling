...
class QwenImageVLMTool(BaseToolWithFileAccess):
    ...
    def inference_with_oai(
        self, image_url, prompt, system_prompt: str | None = None, min_pixels=MIN_PIXELS, max_pixels=MAX_PIXELS
    ) -> str:
    ...
    def call(self, params: Union[str, dict], **kwargs) -> str:
        ...
        if bbox_2d is not None:
            ...
            crop_bbox_2d = [
                int(crop_interpolation_factor * x + (1 - crop_interpolation_factor) * y)
                for (x, y) in zip(whole_bbox_2d, bbox_2d)
            ]
        ...
        response = self.inference_with_oai(image_url, prompt, min_pixels=MIN_PIXELS, max_pixels=MAX_PIXELS)
