# Modified from qwen_agent.utils.output_beautify
# To support display intermediate images in intermediate steps in the tool response
#
...
"""Adapted from qwen_agent.utils.output_beautify

Add support for images encoded in base64 in the tool response.
"""
    ...
    def display_image_if_exists(image_path: str) -> bool:
        ...
        if JUPYTER_AVAILABLE:
            try:
                ...
                ### NEW SUPPORT FOR BASE64 ENCODED IMAGES BASED ON DATA URL
                elif image_path.startswith('data:image'):
                    img = Image.open(BytesIO(base64.b64decode(image_path.split(',')[1])))
                ### END OF NEW SUPPORT FOR BASE64 ENCODED IMAGES BASED ON DATA URL
                else:
                    img = Image.open(image_path)
                display(img)
                return True
            except Exception as e:
                print(f"Error displaying image {image_path[:50]} ...: {e}")
                return False
        return False
                ...
                if part_end_pos > current_pos:
                    ...
                    for image_path in image_list:
                        if not display_image_if_exists(image_path):
                            print(f"Image not found or cannot be displayed: {image_path[:100]} ...")
    
    return full_text


def multimodal_typewriter_print_with_shorten_images(messages: List[dict], text: str = '') -> str:
    """FOR TERMINAL ONLY: Enhanced typewriter print function that displays text and images in Jupyter notebooks."""
